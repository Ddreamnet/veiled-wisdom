...
export default function ListingDetail() {
  ...
  // For product type, get sorted prices
  ...
  const handleBooking = async () => {
    ...
    const {
      data: teacherProfile
    } = await supabase.from("profiles").select("username").eq("user_id", listing.teacher_id).maybeSingle();
  ...
  if (loading) {
    return <div className="container py-8 md:py-12 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          ...
          <div>
            ...
            {listing.cover_url && <div className="relative group overflow-hidden rounded-xl shadow-lg">
                ...
                <img src={getOptimizedCoverUrl(listing.cover_url)} alt={listing.title} loading="lazy" decoding="async" className="w-full h-64 sm:h-80 md:h-96 lg:h-[28rem] object-cover transition-transform duration-500 group-hover:scale-105" />
              </div>}
          </div>
          ...
          <Card className="lg:hidden border-2 shadow-md">
            ...
            <CardContent className="space-y-4 md:space-y-5 p-5 md:p-6">
              <div className="flex items-start gap-3 md:gap-4 pb-3 md:pb-4 border-b">
                {listing.teacher.avatar_url ? <img src={getOptimizedAvatarUrl(listing.teacher.avatar_url)} alt={listing.teacher.username} className="w-12 h-12 md:w-16 md:h-16 rounded-full object-cover flex-shrink-0" /> : <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-primary/20 flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {reviews.length > 0 ? <div className="flex items-center gap-1 text-xs md:text-sm text-muted-foreground">
                      <Star className="w-3 h-3 md:w-4 md:h-4 fill-yellow-400 text-yellow-400" />
                      <span className="font-semibold">{averageRating.toFixed(1)}</span>
                      <span className="text-xs md:text-sm">({reviews.length} değerlendirme)</span>
                    </div> : <div className="text-xs md:text-sm text-muted-foreground">Henüz değerlendirme yok</div>}
                </div>
              </div>

              {listing.teacher.specialization && <div>
              ...
              {listing.teacher.years_of_experience !== undefined && listing.teacher.years_of_experience !== null && <div>
              ...
              <div className="flex flex-col gap-2">
                {user && user.id !== listing.teacher_id && <Button onClick={() => navigate(`/messages?userId=${listing.teacher_id}`)} className="w-full" variant="default">
          ...
          {/* Randevu Kartı - Sadece Video/Messaging için */}
              ...
              <CardContent className="space-y-5 md:space-y-6 p-5 md:p-6">
                ...
                <div className="border-t pt-5">
                  ...
                  <RadioGroup value={selectedDuration?.toString()} onValueChange={v => setSelectedDuration(parseInt(v))} className="space-y-3">
                    {listing.prices.map(price => <div key={price.duration_minutes} onClick={() => setSelectedDuration(price.duration_minutes)} className="flex items-center justify-between p-4 border-2 rounded-xl hover:border-primary/50 hover:bg-primary/5 transition-all cursor-pointer">
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar mode="single" selected={selectedDate} onSelect={setSelectedDate} disabled={date => date < new Date(new Date().setHours(0, 0, 0, 0))} initialFocus locale={tr} className="pointer-events-auto" />
                      </PopoverContent>
                    </Popover>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedTime} onValueChange={setSelectedTime}>
                      ...
                      <SelectContent className="max-h-[300px]">
                        ...
                        return [<SelectItem key={`${hour}:00`} value={`${hour}:00`}>{`${hour}:00`}</SelectItem>, <SelectItem key={`${hour}:30`} value={`${hour}:30`}>{`${hour}:30`}</SelectItem>];
                      }).flat()}
                      </SelectContent>
                    </Select>
                  </div>
                ...
                {selectedPrice && <div className="bg-gradient-to-r from-primary/10 to-primary/5 rounded-xl p-5 border-2 border-primary/20">
                    ...
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="space-y-2 text-base">
                        ...
                        <div className="mt-4 space-y-2 text-foreground">
                          <p>
                            <strong>Tarih:</strong> {selectedDate && format(selectedDate, "dd MMMM yyyy", {
                            locale: tr
                          })}
                          </p>
                          <p>
          ...
          {/* Ürün Satın Alma Kartı - Sadece Product için */}
          {consultationType === "product" && (
            <Card className="border-2 border-primary/20 shadow-lg">
              <CardHeader className="bg-gradient-to-r from-primary/5 to-primary/10">
                <CardTitle className="text-xl md:text-2xl flex items-center gap-2">
              ...
              <CardContent className="space-y-5 md:space-y-6 p-5 md:p-6">
                ...
                <div className="border-t pt-5">
                  ...
                  <RadioGroup value={selectedDuration?.toString()} onValueChange={v => setSelectedDuration(parseInt(v))} className="space-y-3">
                    ...
                    {unitPrice && (
                      <div 
                        onClick={() => setSelectedDuration(unitPrice.duration_minutes)} 
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="space-y-2 text-base">
                        ...
                        <div className="mt-4 space-y-2 text-foreground">
                          <p>
                            <strong>Ürün:</strong> {listing.title}
                          </p>
                          <p>
                            <strong>Adet:</strong> {selectedDuration === 1 ? "1 adet" : `${selectedDuration} adet`}
                          </p>
                          <p>
                            <strong>Tutar:</strong> {selectedPrice?.price} TL
                          </p>
          ...
          <Card className="border-2">
            <CardHeader className="bg-muted/30">
              <CardTitle className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                {reviews.length > 0 && <div className="flex items-center gap-2 text-base md:text-lg">
            ...
            <CardContent>
              {reviews.length === 0 ? <p className="text-sm md:text-base text-muted-foreground text-center py-6 md:py-8">
                  Henüz yorum bulunmuyor.
                </p> : <div className="space-y-4 md:space-y-6">
                  {reviews.map(review => <div key={review.id} className="border-b last:border-0 pb-4 md:pb-6 last:pb-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start sm:items-center justify-between gap-2 mb-1">
                            ...
                            <div className="flex items-center gap-0.5 flex-shrink-0">
                              {Array.from({
                          length: 5
                        }).map((_, i) => <Star key={i} className={`w-3 h-3 md:w-4 md:h-4 ${i < review.rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"}`} />)}
                            </div>
                          </div>
