...
export default function CategoryDetail() {
  ...
  const fetchCategory = async () => {
    ...
    const {
      data: categoryData
    } = await supabase.from("categories").select("*").eq("slug", slug).is("parent_id", null).single();
    if (!categoryData) {
      setLoading(false);
      return;
    }
    ...
    } = await supabase.from("categories").select("*").eq("parent_id", categoryData.id).order("display_order", {
      ascending: true
    });
    if (subCategoriesData) {
      ...
      for (const subCat of subCategoriesData) {
        const {
          ...
          count: "exact",
          head: true
        }).eq("category_id", subCat.id).eq("is_active", true);
  ...
  return <div className="container py-8 md:py-12 px-4">
      ...
      {subCategories.length === 0 ? <Card>
        ...
        </Card> : <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
