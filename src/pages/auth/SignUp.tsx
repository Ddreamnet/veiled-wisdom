...
export default function SignUp() {
  ...
  return <div className="min-h-screen flex items-center justify-center p-4 py-12 liquid-gradient">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="username" value={username} onChange={e => setUsername(e.target.value)} required className="glass-effect border-silver/20" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="email" className="text-silver-muted">E-posta</Label>
              <Input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} required className="glass-effect border-silver/20" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="password" className="text-silver-muted">Şifre</Label>
              <Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} required className="glass-effect border-silver/20" />
            </div>
            <div className="space-y-2">
            ...
            {role === 'teacher' && <>
                <div className="space-y-2">
                  ...
                  <Input id="dateOfBirth" type="date" value={dateOfBirth} onChange={e => setDateOfBirth(e.target.value)} required max={new Date().toISOString().split('T')[0]} className="glass-effect border-silver/20" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="phone" className="text-silver-muted">Telefon *</Label>
                  <Input id="phone" type="tel" placeholder="05XX XXX XX XX" value={phone} onChange={e => setPhone(e.target.value)} required maxLength={20} className="glass-effect border-silver/20" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="specialization" className="text-silver-muted">Uzmanlık Alanı *</Label>
                  <Input id="specialization" value={specialization} onChange={e => setSpecialization(e.target.value)} required maxLength={200} className="glass-effect border-silver/20" placeholder="\xD6rn: Bak\u0131m, astroloji, vefk" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="education" className="text-silver-muted">Eğitim Bilgisi *</Label>
                  <Textarea id="education" placeholder="Mezun olduğunuz okul ve bölüm bilgileri" value={education} onChange={e => setEducation(e.target.value)} required rows={3} maxLength={500} className="glass-effect border-silver/20" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="yearsOfExperience" className="text-silver-muted">Deneyim (Yıl) *</Label>
                  <Input id="yearsOfExperience" type="number" min="0" max="70" value={yearsOfExperience} onChange={e => setYearsOfExperience(e.target.value)} required className="glass-effect border-silver/20" />
                </div>
              </>}
