...
export default function Approvals() {
  ...
  const handleRepair = async (approval: TeacherApproval) => {
    ...
    try {
      ...
      if (approval.hasProfileIssue) {
        ...
        if (profileError) {
          // Profile might already exist (race condition), try update instead
          if (profileError.code === '23505') {
            await supabase
              .from('profiles')
              .update({
                username: approval.full_name || undefined,
                is_teacher_approved: approval.status === 'approved',
              })
      ...
      if (approval.hasRoleIssue && approval.status === 'approved') {
        ...
        // Update profile is_teacher_approved
        await supabase
          .from('profiles')
  ...
  const handleApproval = async (approvalId: string, userId: string, approve: boolean) => {
    ...
    if (approve) {
      ...
      if (!existingProfile) {
        // Get approval data to create profile
        ...
        const { error: profileCreateError } = await supabase
          ...
          .insert([{
            id: userId,
            username: approvalData?.full_name || 'Kullanıcı',
          ...
          {needsRepair && (
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3 text-sm">
              ...
              <ul className="list-disc list-inside text-muted-foreground space-y-1">
                ...
                {approval.hasRoleIssue && <li>Hoca rolü atanmamış</li>}
              </ul>
              <Button
                size="sm"
                variant="outline"
  ...
  const repairNeededCount = [...pendingApprovals, ...approvedApprovals, ...rejectedApprovals]
    .filter(a => a.hasProfileIssue || a.hasRoleIssue).length;
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex gap-2">
            {repairNeededCount > 0 && (
              <Badge variant="outline" className="text-base px-3 py-1 bg-amber-500/10 text-amber-600 border-amber-500/30">
