...
export function CategoryImageUpload({ 
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (url) {
      toast({
        title: 'Başarılı',
        description: 'Kategori görseli başarıyla yüklendi.',
      });
  ...
  return (
    <div className="space-y-4">
      {previewUrl ? (
        ...
        <div className="w-full h-48 border-2 border-dashed border-border rounded-lg flex flex-col items-center justify-center gap-4 hover:border-primary/50 transition-colors">
          <Upload className="h-12 w-12 text-muted-foreground" />
          <p className="text-sm text-muted-foreground">Kategori görseli yükle</p>
        </div>
      )}
